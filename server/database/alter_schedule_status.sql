-- ============================================================
-- 일정 캘린더에 상태(event_status) 추가
--   NORMAL    : 예정 (기본)
--   COMPLETED : 완료  -> 캘린더에서 취소선으로 표시
--   CANCELLED : 취소  -> 캘린더에서 빨간색으로 표시
-- ============================================================

USE jysk;

-- 1. schedule_event 테이블에 event_status 컬럼 추가
ALTER TABLE schedule_event
ADD COLUMN event_status VARCHAR(20) NOT NULL DEFAULT 'NORMAL'
    COMMENT '일정 상태 (NORMAL: 예정, COMPLETED: 완료, CANCELLED: 취소)'
AFTER is_important;

-- 2. v_schedule_event_detail 뷰 재생성 (event_status 포함)
